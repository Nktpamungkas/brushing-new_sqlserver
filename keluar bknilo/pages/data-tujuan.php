...
<body>
  ...
  if (isset($_POST['btnHapus'])) {
    $hapusSql = "DELETE FROM tbl_tujuan WHERE id='$_POST[id]'";
  ...
  if (isset($_POST['btnSimpan'])) {
    ...
    $simpanSql = "INSERT INTO tbl_tujuan SET 
	`tujuan`='$tujuan',
	`ket`='$ket'";
  ...
  if (isset($_POST['btnUbah'])) {
    $tujuan = $_POST['tujuan'];
    $ket = str_replace("'", "", $_POST['ket']);
    $simpanSql = "UPDATE tbl_tujuan SET 
	`tujuan`='$tujuan',
	`ket`='$ket'
	WHERE `id`='$_POST[id]'";
  ...
  <form id="form1" name="form1" method="post" action="" enctype="multipart/form-data">
    <table width="100%" border="0">
      ...
      <?php $qtampil = mysqli_query($con,"SELECT * FROM tbl_tujuan WHERE tujuan='$_GET[tujuan]' LIMIT 1");
    ...
    <table width="100%" border="0">
      ...
      <?php
      $qry = mysqli_query($con,"SELECT * FROM tbl_tujuan ORDER BY id ASC");
      $no = 1;
