...
<body>
  ...
  if (isset($_POST['btnHapus'])) {
    $hapusSql = "DELETE FROM tbl_dyestuff WHERE id='$_POST[id]'";
  ...
  if (isset($_POST['btnSimpan'])) {
    ...
    $simpanSql = "INSERT INTO tbl_dyestuff SET 
	`kode`='$kode',
	`ket`='$ket'";
  ...
  if (isset($_POST['btnUbah'])) {
    $kode = $_POST[kode];
    $ket = str_replace("'", "", $_POST[ket]);
    $simpanSql = "UPDATE tbl_dyestuff SET 
	`kode`='$kode',
	`ket`='$ket'
	WHERE `id`='$_POST[id]'";
  ...
  <form id="form1" name="form1" method="post" action="" enctype="multipart/form-data">
    <table width="100%" border="0">
      ...
      <?php $qtampil = mysql_query("SELECT * FROM tbl_dyestuff WHERE kode='$_GET[kode]' LIMIT 1");
    ...
    <table width="100%" border="0">
      ...
      <?php
      $qry = mysql_query("SELECT * FROM tbl_dyestuff ORDER BY id ASC");
      $no = 1;
