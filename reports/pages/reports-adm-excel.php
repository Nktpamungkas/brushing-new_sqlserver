...
<body>
        ...
        <table width="100%" border="0">
            ...
            <?php
                $sql = sqlsrv_query($con, " SELECT 
                                    *, a.id as idp,
                                    SUBSTR(a.langganan, LOCATE('/', a.langganan)+1)	AS buyer
                                  FROM
                                    db_brushing.tbl_adm a
                                  WHERE
                                    a.status='1' AND " . $tgl . $shift . " ORDER BY a.tgl_in ASC ");
                $no = 1;
    ...
    <form id="form1" name="form1" method="post" action="">
        ...
        <table width="100%" border="0">
            ...
            <?php
                    $sql = sqlsrv_query($con, " SELECT 
                                        *,a.id as idp ,DATEDIFF(a.tgl_out,a.tgl_in) as Hari, TIMEDIFF(a.tgl_out,a.tgl_in) as Jam,
                                        SUBSTR(a.langganan, LOCATE('/', a.langganan)+1)	AS buyer 
                                      FROM
                                        db_brushing.tbl_adm a
                                      WHERE
                                        a.status='2' AND " . $tgl . $shift . " ORDER BY a.tgl_in ASC ");
                    $no = 1;
            ...
            <?php
                    $sql = sqlsrv_query($con, " SELECT 
                                ...
                                FROM
                                  db_brushing.tbl_adm a
                                WHERE
                                  (a.status='1' OR a.status='2') AND " . $tgl . $shift . " ORDER BY a.tgl_in ASC ");
                    $no = 1;
