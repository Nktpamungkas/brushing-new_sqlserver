...
<?php
	...
	$mc=$_GET['no_mesin'];
	$opr=$_GET['opr'];
		if($tglakhir != "" and $tglawal != ""){$tgl=" DATE_FORMAT(a.`tgl_update`,'%Y-%m-%d') BETWEEN '$tglawal' AND '$tglakhir' ";}else{$tgl=" ";}
		if($shft=="ALL"){$shift=" ";}else{$shift=" AND a.`shift`='$shft' ";}
		if($nmesin!="ALL"){$mesin=" AND a.`nama_mesin`='$nmesin'";}else{$mesin=" ";}
	    if($mc!="ALL"){$nomesin=" AND a.`no_mesin`='$mc'";}else if($mc=="ALL"){$nomesin=" ";}else{$nomesin=" ";}
		if($opr == "ALL"){ $wopr = " "; } else { $wopr = " AND a.`acc_staff`='$opr' "; }
?>
...
WHERE
	DATE_FORMAT(a.`tgl_update`,'%Y-%m-%d') BETWEEN '$tglawal' AND '$tglakhir' ".$shift.$mesin.$nomesin.$wopr." ORDER BY a.`no_mesin` ASC");
...
?></td>
      ...
      <td><?php
                    ...
                    echo  $pjam * (60 * 60);
                    ?></td>
      <td><?php if ($rowd['waktu_proses']>0){echo round((($pjam * (60 * 60))/round($rowd['waktu_proses'],2))*100,2);}else{ echo "0";} ?></td>
    </tr>
     <?php 
	 $totrol +=$rowd['rol'];
	 $totberat +=$rowd['qty'];
